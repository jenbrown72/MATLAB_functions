function [data] = JB_sessionAnalysisTEST(filename,fileSession,PreStim,PostStim)
...
load('DATA.mat')
toAnalyseDATA = DATA.(filename);
NumberToAnalyse = size((toAnalyseDATA),2);

if nargin==2 %if fileSession was also chosen
    
    tempDATA = toAnalyseDATA{fileSession}; %just focus in on this session
    NumberToAnalyse = 1;
end

for kk = 1:NumberToAnalyse %go through and analysis all specified sessions
    
    
    if nargin==2
        sessionNumber=fileSession; %if a file was specified make sure the data is saved in the correct field
    else
    sessionNumber = kk;
    tempDATA = toAnalyseDATA{sessionNumber};
    end
...
DATA.(filename){sessionNumber}.analysedDATA = data;
end
