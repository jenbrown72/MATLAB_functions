function [DATA, loadedFiles] = JB_loadTxtFileExp(MouseID)
...
if nargin==1; % if a filename was inputted into function
    ...
    tempdataDir = char(strcat(startDir, currfile, '\expDatatxtFilesTrimming'));
    cd(tempdataDir);
...
currDir = cd;
dataDir = strcat(currDir,'\expDatatxtFiles\expData');
cd(dataDir);
...
curr = cd;
cd(strcat(curr, '\expDatatxtFiles\data'))
